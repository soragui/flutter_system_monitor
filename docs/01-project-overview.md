...
虽然当前只编译 Linux，但 UI 层（screens/widgets/providers）与平台无关。未来扩展到 Windows/macOS 时，只需要替换 `services/system_monitor.dart` 中的平台数据采集逻辑，其余代码无需改动。

> 这是 Flutter 桌面应用的核心价值主张：**UI 逻辑跨平台复用，平台特定逻辑隔离在 service 层**。
...
**只有 3 个业务依赖**。这不是巧合，这是刻意的设计选择（详见 [08 设计决策](08-design-decisions.md)）。
...
下一章：[02 架构全景](02-architecture-overview.md) — 四层架构如何组织，数据如何从 Linux 内核流向屏幕上每一个像素。
