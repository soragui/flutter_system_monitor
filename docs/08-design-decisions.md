...
不是"来不及加"，而是**刻意不加**。每个被排除的依赖都有一个理由：
...
> **原则**：每个依赖都是一笔债务。需要跟踪版本更新、处理破坏性变更、理解 API 设计哲学。700 行代码的项目，3 个依赖够了。
...
不是一个"大"项目，但是一个**完整**的项目。它展示了一个桌面应用应该怎么做，以及更重要的是——不应该做什么。
