...
这个项目选择 Flutter 而非传统的 GTK/Qt 等 Linux 原生工具链，基于以下考虑：

1. **跨平台潜力**：同一套代码可编译到 Linux/Windows/macOS，未来扩展成本低
2. **声明式 UI**：Widget 树天然适合数据驱动的监控面板，状态变化自动触发 UI 更新
3. **热重载**：开发迭代速度快，修改 UI 即刻看到效果
4. **Material Design 3**：内置暗色主题支持，视觉效果现代统一
...
Linux 的 `/proc` 虚拟文件系统提供了几乎所有系统监控数据的标准接口。直接读取 `/proc` 相比引入第三方库有以下优势：

- **零依赖**：不需要额外安装系统级软件包（如 `lm-sensors`、`sysstat`）
- **高性能**：`/proc` 是内核内存映射文件，读取开销极低
- **可移植性**：所有主流 Linux 发行版都提供 `/proc`
- **精确控制**：可以精确获取所需字段，避免冗余数据解析
...
Provider 是 Flutter 官方推荐的状态管理方案。对于本项目这种"单一数据源 → 多个 UI 组件"的场景，Provider 是最合适的：

- **ChangeNotifier** 提供了最简单的响应式编程模型
- **Consumer Widget** 精确控制重建范围，避免不必要的 rebuild
- **与 Flutter 深度集成**，不需要额外引入 Redux/BLoC 的概念负担
...
**踩坑记录 — 无限宽度错误**：
在 Row 中嵌套 Column 再嵌套 LinearProgressIndicator 时，Column 从 Row 获得的是无界宽度（`BoxConstraints(w=Infinity)`），而 LinearProgressIndicator 内部尝试填充所有可用宽度，导致抛出 "BoxConstraints forces an infinite width" 异常。解决方案是用 `SizedBox(width: ...)` 给 LinearProgressIndicator 赋予确定的宽度约束。
...
这不是"偷懒"，而是**在项目复杂度允许范围内选择最简单的方案**。当项目只有一个页面、四种数据类型时，引入 DI 容器或代码生成的边际收益远低于认知负担。
...
核心原则：**已有数据时，单次采集失败不应清空整个界面**。这确保了即使网络接口偶尔无法读取或磁盘临时卸载，用户仍能看到之前采集的有效数据。
